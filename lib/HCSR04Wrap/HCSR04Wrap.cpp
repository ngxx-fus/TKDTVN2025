/* ===================================================
 * FILE: lib/HCSR04Wrap/HCSR04Wrap.cpp
 * (Đã sửa lỗi 'def' VÀ thêm header còn thiếu)
 * ===================================================*/

#include "HCSR04Wrap.h"
#include <Arduino.h>       // <-- THÊM DÒNG NÀY (Để lấy GPIO, gpio_config_t)
#include "driver/gpio.h" // <-- Thêm dòng này cho an toàn

/// #pragma message("[compile] HCSR04Wrap.cpp")
...
// +++ SỬA LỖI Ở ĐÂY: "def" -> "DEFAULT_RETURN_STATUS" +++
...
DEFAULT_RETURN_STATUS hcsr04MeasureAll(){
    ...
    for(int i = 0; i < HCSR04_SERSOR_NUM; ++i){
        ...
        while(1){
            ...
            if(esp_timer_get_time() - timeHigh > HCSR04_WAIT_FOR_HIGH) {
                __sys_err("[hcsr04MeasureAll] Loop-%d: WaitingForHigh timeout!", i); // Sửa %lld thành %d
                /// Error occured!
                return STATUS_ERR;
            }
        }
        ...
        while(1){
            ...
            if((timeLow = esp_timer_get_time()) - timeHigh > HCSR04_WAIT_FOR_LOW) {
                /// Error occured!
                __sys_err("[hcsr04MeasureAll] Loop-%d: WaitingForLow timeout(%lld)!", i, (timeLow - timeHigh)); // Sửa %lld thành %d
                return STATUS_ERR;
            }
        }
        hcsr04Data.arr[i] = ((timeLow - timeHigh) * 17) / (1000);
